  ...
  //% shim=sendBufferAsm
    ...
    export class Strip {
        ...
        showBarGraph(value: number, high: number): void {
            if (high <= 0) {
                this.clear();
                this.setPixelColor(0, HaodaBitColors.Yellow);
                this.show();
                return;
            }
            ...
            if (v == 0) {
                ...
                for (let i = 0; i < n; ++i) {
                    if (i <= v) {
                        ...
                        this.setPixelColor(i, HaodaBit.rgb(b, 0, 255 - b));
                    }
	 ...
	 * @param chain type of matrix
         ...
         */
        //% blockId="HaodaBit_length" block="%strip|length" blockGap=8
         ...
         */
        //% weight=9 blockId=HaodaBit_power block="%strip|power (mA)"
        //% advanced=true
        power(): number {
            const stride = this._mode === HaodaBitMode.RGBW ? 4 : 3;
            const end = this.start + this._length;
            ...
            return Math.idiv(this.length(), 2) /* 0.5mA per HaodaBit */
                + Math.idiv(p * 433, 10000); /* rought approximation */
        }

        private setBufferRGB(offset: number, red: number, green: number, blue: number): void {
            if (this._mode === HaodaBitMode.RGB_RGB) {
        ...
        private setPixelRGB(pixeloffset: number, rgb: number): void {
            ...
            let stride = this._mode === HaodaBitMode.RGBW ? 4 : 3;
    ...
    export function create(pin: DigitalPin, numleds: number, mode: HaodaBitMode): Strip {
        let strip = new Strip();
        let stride = mode === HaodaBitMode.RGBW ? 4 : 3;
    ...
    */
    ...
    //% advanced=true
    export function colors(color: HaodaBitColors): number {
        return color;
    }
     ...
     */
    //% blockId=HaodaBitHSL block="hue %h|saturation %s|luminosity %l"
